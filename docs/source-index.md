...
| `2026-06-08` | `source/2026-06-08-imedtac-teams-cors-preflight-block/` | Teams incident report where Johnny Fang said imedtac's formal-machine test showed an error after completing measurement and could not enter the Triage flow. The attached DevTools screenshot confirms a browser CORS preflight block: the request to `POST https://nycu-imedtac-triage-demo-api.onrender.com/api/triage-demo/sessions` came from origin `http://127.0.0.1:5174`, while the NYCU API allowlist currently covers `http://localhost` and `http://localhost:5174`. The browser blocked the request before the NYCU session API could start, so the evidence points to missing CORS allowlist coverage for `127.0.0.1`, not to a broken endpoint schema, question logic failure, bearer-token failure, or confirmed backend timeout. Recommended fix: add `http://127.0.0.1:5174`, redeploy Render, verify OPTIONS preflight, then ask imedtac to retest the same measurement-to-triage transition. |
