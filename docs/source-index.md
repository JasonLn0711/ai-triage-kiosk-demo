...
| `2026-05-26` | `source/2026-05-26-imedtac-teams-summary-preview-followup/` | Teams follow-up where Johnny clarifies that, in the real scenario, the summary content goes into HIS and there is no separate preview content; the preview need exists because the demo needs a convenient way to introduce the result. Johnny suggests using previously provided NYCU prototype / final-screen material as support, so 慧誠智醫 does not need to build a separate new preview page right now. This source also records Jason's `2026-05-26 13:52` Teams message sending the NYCU demo summary page image with `畫面大致呈現如上`, Johnny's confirmation that it is sufficient for explanation, Ben's request for an iframe URL, the QR-code-page replacement direction, and Jason's `2026-05-26 17:52` Teams reply providing the fixed Render summary URL plus `window.name` handoff while preserving the two-endpoint API contract. |
| `2026-05-27` | `source/2026-05-27-duobao-line-implementation-sync/` | Internal LINE sync between Jason and 多寶 after the imedtac Teams UI / RWD follow-up. Records 多寶's workload check and offer to divide work, Jason's repo / thinking-spec update, the completed human-factor option-label and partial-vitals implementation summary, GitHub push note, mobile responsive screenshots, what Jason already told imedtac in Teams, the clarification that current work is the case demo first, the future discussion topic of AI-generated suitable options, and the hosting/code-sharing boundary: imedtac sees the Render API / demo service, while the code repo itself has not been shared. Also records that future 多寶 code edits can go through PR-style review and that final-exam timing may slow responses. |
| `2026-05-27` | `source/2026-05-27-imedtac-teams-ui-option-human-factor/` | Teams follow-up where Johnny Fang shared imedtac's UI / human-factor option constraints and Jason replied that NYCU would adjust generated content accordingly. This record refines the earlier no-scroll option capacity into a concrete content contract: `3 x 3` grid, `2-9` options, multi-choice labels around `26` total characters, and single-choice labels around `60` total characters. The same Teams thread also records Johnny's partial / single-item measurement question, 多寶's rule that missing vital signs should not be considered in question design, Johnny's confirmation that partial data can still run the complete question flow, and Lauren Wang's report that the API flow has no major issue before integrated UI review. Derived decisions: `decisions/2026-05-27-imedtac-ui-option-content-contract.md` and `decisions/2026-05-27-imedtac-partial-vitals-question-flow-contract.md`. |
| `2026-05-27` | `source/2026-05-27-imedtac-teams-summary-rwd-qr-followup/` | Teams follow-up where Johnny asked whether the final result preview page has basic RWD because imedtac may scan a QR code to view the result. Jason asked whether the demo day needs mobile preview and whether NYCU needs to coordinate any specific process. Johnny clarified that, if the original flow is unchanged, users will scan a QR code, imedtac is also considering directly clicking the QR code to open a new web page for demo explanation, and NYCU only needs to stay online on standby for troubleshooting after imedtac confirms the time. Jason then replied that three phone/tablet layouts had been tested without bleeding / overflow, attached screenshots, summarized the human-factor option-label shortening, confirmed the structural contract stayed unchanged, and recorded partial-vital testing. Johnny acknowledged the update. Ben later raised the QR implementation constraint: `window.name` does not work cleanly for phone QR scanning from another browser context, the full payload is too large for a QR URL, and imedtac wants to know whether a URL with only `session_key` can display the summary page and how long that key is valid. Jason replied that NYCU's current direction is to preserve Endpoint 1 / Endpoint 2, add a read-only summary lookup for QR code use, let iMVS generate `/demo-ui/summary-review/?session_key=<session_key>` after `status=summary`, align validity with `session_expires_at` / current `30` minute demo TTL, and display summary only when `session_state=summary_ready` and the key is not expired. The repo implements this light read-only summary lookup bridge for QR use: `/demo-ui/summary-review/?session_key=<session_key>` backed by `GET /api/triage-demo/sessions/{session_key}/summary`, with URL-safe random session keys and the same expiry answer. |
| `2026-06-02` to `2026-06-05` | `source/2026-06-02-to-2026-06-05-imedtac-teams-question-design-and-screening-scope/` | Teams follow-up where Johnny reported that imedtac's flow is connected and on the formal machine for testing, accepted NYCU's single test/demo environment for both imedtac test and formal machines, and confirmed the demo story remains the high-heart-rate / tachycardia scenario. Johnny's test feedback is now an active demo expectation: the bounded tachycardia flow should show visible dynamic interaction for different data / answers, and the result page values should match the measured session values. The exchange also records the timing target: `2026-06-10` was discussed but not confirmed; the working latest first version is before `2026-06-15`, with gradual iteration acceptable if imedtac is notified before deployment. The same source introduces a separate proposed 北市聯醫 health-cabin lane for vision/hearing screening, question-bank / upload-system design, and future data/HIS integration; treat that as proposal discovery with clinical/device-owner review, not as part of the June AI-triage demo. Derived response plan: `docs/2026-06-05-question-design-and-screening-scope-response-plan.md`. |
| `2026-06-08` | `source/2026-06-08-imedtac-teams-cors-preflight-block/` | Teams incident report where Johnny Fang said imedtac's formal-machine test showed an error after completing measurement and could not enter the Triage flow. The attached DevTools screenshot confirms a browser CORS preflight block: the request to `POST https://nycu-imedtac-triage-demo-api.onrender.com/api/triage-demo/sessions` came from origin `http://127.0.0.1:5174`, while the NYCU API allowlist covered `http://localhost` and `http://localhost:5174`. The browser blocked the request before the NYCU session API could start, so the evidence pointed to missing CORS allowlist coverage for `127.0.0.1`, not to a broken endpoint schema, question logic failure, bearer-token failure, or confirmed backend timeout. After adding `http://127.0.0.1:5174` and redeploying Render, NYCU verification showed: start-session preflight returned `204`, answers-endpoint preflight returned `204`, no-token POST reached API-level `401 demo_bearer_token_required`, `/demo-ui/summary-review/` returned `200`, and a headless Chromium browser page served from `http://127.0.0.1:5174/` reached API-level `401` instead of being blocked by CORS. Authenticated full-session verification was not run from the NYCU computer because no private demo bearer token was present locally. Next gate: imedtac retests the same measurement-to-triage transition; if the generic modal persists, inspect bearer-token delivery, stale frontend bundle/cache, actual browser origin, request payload shape, and Render logs. |
...
- `docs/2026-05-27-ui-option-content-audit-and-next-plan.md`
