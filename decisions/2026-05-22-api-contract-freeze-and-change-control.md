...
| `2026-05-27 internal LINE` | Jason and 多寶 synchronized the implementation state: option-label shortening is display-only, partial-vitals testing preserves existing API shape, the current work is the case demo first, imedtac sees the Render API / demo service but not the code repo, future 多寶 edits can use PR-style review, and AI-generated suitable options remain a future internal discussion. | Keep internal design ideas separate from imedtac-facing commitments. Do not expose AI-generated option behavior, code-repo access, Cloudflare/lab-GPU routing, or clinical wording changes externally without a recorded decision and explicit imedtac / 許醫師 review where applicable. |
| `2026-05-27 15:42-16:01` | Johnny asked whether the final result preview page has basic RWD for QR-code viewing; Jason asked about mobile preview and support workflow; Johnny clarified QR-code direct viewing / possible direct-click new-page demo introduction and online-standby support; Jason replied that three phone/tablet layouts were tested without bleeding, attached screenshots, summarized option-label shortening, and confirmed partial-vital testing behavior. | Keep the summary-review page usable through QR-code direct navigation on phone/tablet; preserve mobile RWD after visual edits; keep option-label shortening display-only unless imedtac is told otherwise; keep missing vital signs out of question routing and summary facts; treat NYCU's default demo-day role as online standby until imedtac confirms another support flow. |
| `2026-05-27 17:55` | Ben replied that PM needs QR Code phone scanning to open the summary page, `window.name` does not fit QR scanning from another browser context, and the response payload is too large to put into a QR URL. Ben asked whether a URL containing `session_key` can display the summary page and how long the key is valid. | Treat URL `session_key` summary display as an added read-only summary bridge, not a change to the two POST question-loop endpoints. The implementation preserves Endpoint 1 / Endpoint 2, uses `session_expires_at` as the expiry source, generates URL-safe random session keys, and adds `GET /api/triage-demo/sessions/{session_key}/summary` for completed summaries. Tell imedtac this behavior before they implement against it as a shared dependency. |
| `2026-05-27 after Ben QR question` | Jason replied to Ben in Teams: `保留 Endpoint 1 / Endpoint 2 不變`, add a read-only `summary lookup` for QR code use, let iMVS generate `https://nycu-imedtac-triage-demo-api.onrender.com/demo-ui/summary-review/?session_key=<session_key>` after `status=summary`, let the summary page read the completed `staff_review_summary` from NYCU backend, align validity with `session_expires_at` (`30` minutes in current demo runtime), display only when `session_state=summary_ready` and the key is not expired, and asked `Ben 覺得這樣如何？` | This is now an external proposed integration direction, not only an internal design. Preserve the exact endpoint-stability promise, URL pattern, expiry rule, and readiness/expiry display condition unless Ben / imedtac replies with a recorded revision. |
