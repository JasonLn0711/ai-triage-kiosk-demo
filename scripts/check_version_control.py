...
def main() -> int:
    ...
    engine_text = (ROOT / "core" / "triage_engine" / "index.js").read_text(encoding="utf-8")
    if f'versionLabel: "{expected_label}"' not in engine_text:
        errors.append(f"core/triage_engine/index.js does not expose {expected_label}")

    html_text = (ROOT / "app" / "triage-kiosk" / "index.html").read_text(encoding="utf-8")
    if f'id="versionBadge">{expected_label}</span>' not in html_text:
        errors.append(f"app/triage-kiosk/index.html fallback badge is not {expected_label}")
