...
The first two POST routes are the unchanged question-loop contract. The GET
route is a read-only QR-display helper for completed summaries.
...
OPTIONS /api/triage-demo/sessions/{session_key}/summary
...
- `GET /healthz`, `GET /api/triage-demo/sessions/{session_key}/summary`, and
  `OPTIONS` preflight do not require the bearer token. The read-only summary
  route treats the short-lived random `session_key` as the QR viewing
  capability for synthetic demo summaries.
...
For same-tab direct navigation, iMVS can continue to use the existing
`window.name` handoff before navigating to:

```text
https://nycu-imedtac-triage-demo-api.onrender.com/demo-ui/summary-review/
```

For phone QR scanning from another browser context, use the completed session's
`session_key`:
...
- `session_key` is generated by the NYCU backend as a URL-safe random token.
- Render hosts the service; Render does not issue or manage the token.
- The key is valid until `session_expires_at`, currently `30` minutes from
  session creation in the demo runtime.
- Current demo storage is the Node process in-memory session map, so active
  QR links are expected to expire early if the Render instance restarts. Redis /
  KV / DB persistence can be added later if imedtac needs restart-tolerant or
  longer-lived summary links.
- The read-only summary route returns summary only after
  `session_state=summary_ready`.
- Unknown keys return `404 invalid_session`; active sessions return
  `409 session_not_summary_ready`; expired sessions return
  `410 session_expired`.
- Full summary payloads, bearer tokens, and private values are not placed in the
  QR URL.
...
- `status = "question"`
- `session_key` present as a URL-safe random token generated by NYCU backend
- `response_id` present
- `progress.current = 1`
