...
- Jason prepared a `DEMO_BEARER_TOKEN` value in Render for token-required
  ...
  behavior is now active on Render. Public verification on
  `2026-05-25 20:25 GMT+8` confirmed `/healthz` HTTP `200`, CORS preflight HTTP
  `204`, and no-token start-session HTTP `401` /
  `demo_bearer_token_required`; private bearer-token success was shared through
...
| 4 | Summary display direction: imedtac does not need to build a new page; NYCU provides the final prototype / summary-review screen material plus payload examples and wording. Same-Render route target: `https://nycu-imedtac-triage-demo-api.onrender.com/demo-ui/summary-review/`. | imedtac UI + NYCU | Option B pushed to GitHub `main`; public Render route still returned HTTP `404` through `2026-05-26 16:20` Asia/Taipei, so trigger Render manual deploy and verify before notifying imedtac |
...
5. Summary preview
回答完所有問題後，NYCU Endpoint 2 會回 status=summary 與 staff_review_summary。最省工程量的方式是 iMVS 直接在既有 result / preview page 顯示這份 payload；如果你們想先快速驗證畫面，我們也可以討論一個 NYCU-hosted demo-only preview / mock page，但正式 rehearsal 建議還是以 iMVS 端顯示同一份 summary payload 為主。若要改成 NYCU-provided UI，因為可能影響畫面一致性與設備操作完整性，我們會先跟你們討論後再調整。
...
實際 demo token 不會放在群組訊息、文件、GitHub、截圖或 log 裡，會另外透過私下通道提供給貴司工程端。GET /healthz 與 OPTIONS preflight 不需要 token；兩個 POST endpoints 已在 token-required mode 啟用後要求 bearer token。
...
我們也會把第一版 tachycardia preset questions/options 對齊多寶 / 許醫師今天提供的 case 後整理成工程可用的交付包。這裡的「給大家」包含兩層：第一，NYCU backend / mock API 會以這組 question ids、option ids 與 summary payload 作為第一輪 rehearsal 行為；第二，NYCU 會把同一組 preset questions/options、API examples、expected answer path 與 staff_review_summary 範例整理成文件，提供給 Ben、Johnny、Lauren、imedtac UI/engineering team，以及多寶 / 許醫師確認 wording 與 clinical boundary。
```
