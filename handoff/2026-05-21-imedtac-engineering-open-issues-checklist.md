...
- QR summary viewing uses an added read-only display helper:
  - `GET /api/triage-demo/sessions/{session_key}/summary`
  - This helper does not change the two POST question-loop endpoints.
- 六月 default values：
...
| P0-03 | Closed for mock. Sessions return URL-safe random `session_key`, `session_expires_at`, `session_state=active`, then `summary_ready`. New answers after summary return `session_summary_ready`; invalid keys return `invalid_session`. QR summary viewing uses the added read-only helper `GET /api/triage-demo/sessions/{session_key}/summary`, which returns summary only after `summary_ready`, `409` while still active, `404` for unknown keys, and `410` after expiry. | `api/lib/triage-demo-contract.js`; `api/triage-demo/sessions/[session_key]/summary.js`; `tests/contract/triage-demo-api.test.js`. |
...
| P1-06 | Staff-summary display location | Johnny asked whether NYCU already has a summary preview page after all questions. On `2026-05-26`, Johnny clarified that the real scenario sends summary content into HIS and has no preview content; the preview need is for demo explanation. Since Johnny has already seen the NYCU prototype, 慧誠智醫 does not need to build a separate new page now and can use NYCU-provided prototype / final-screen material as support. Ben later asked for a QR-friendly `session_key` path because phone scanning cannot inherit `window.name`; NYCU added this as a read-only display helper. | imedtac UI + NYCU | immediate | UI copy 不含 diagnosis、treatment、final triage level、production HIS/EMR claim；summary remains `staff_only`; the NYCU-hosted summary review UI remains demo-only; QR lookup does not change the two POST question-loop endpoints。 |
...
- QR summary URL can show the completed summary with
  `/demo-ui/summary-review/?session_key=<session_key>` after
