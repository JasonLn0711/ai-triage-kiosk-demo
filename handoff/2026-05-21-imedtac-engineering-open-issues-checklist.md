...
| P0-07 | Closed for first rehearsal path at the code/contract layer. CORS allows browser origins `http://localhost` and `http://localhost:5174`, supports `OPTIONS`, and the runtime now supports an environment-controlled `DEMO_BEARER_TOKEN` gate for `Authorization: Bearer <demo token>` without storing a token. Jason prepared the Render environment variable on `2026-05-25`; post-push public verification still returned HTTP `200` for no-token start-session, so the remaining Render action is save/rebuild/redeploy and then verify HTTP `401` / `demo_bearer_token_required`. The NYCU-hosted Render rehearsal base URL is fixed as `https://nycu-imedtac-triage-demo-api.onrender.com/api/triage-demo`; this is a deployment target, not a schema change. | `api/lib/triage-demo-contract.js`; `api/triage-demo/sessions.js`; `api/triage-demo/sessions/[session_key]/answers.js`; `handoff/2026-05-25-first-rehearsal-packet.md`; `handoff/2026-05-25-render-rehearsal-api-deployment-runbook.md`. |
