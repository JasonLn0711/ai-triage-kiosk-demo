...
page or NYCU can show in a temporary demo-only preview during rehearsal. Current
runtime `session_key` values are URL-safe random tokens generated by the NYCU
backend; older example strings such as `demo-session-tachy-001` are placeholders
for "use the session key returned by Endpoint 1", not predictable runtime keys.
For phone QR scanning, iMVS can use
`/demo-ui/summary-review/?session_key=<session_key>` after the session reaches
`status=summary`; the page reads the completed summary through the read-only
`GET /api/triage-demo/sessions/{session_key}/summary` helper.
