...
| `vitals.<field>.value` | number/null | 單一 vital 欄位的量測值；若 unavailable、failed 或不適用，可為 `null`。 | 範例：`heart_rate_bpm.value = 150`；代表 tachycardia live demo 的心跳量測值。 |
...
| `progress.expected_total` | number | yes | 預估總題數；動態流程可為估計值，但應維持在 demo 題數上限內。 | 範例：`5`；代表本 session 預計約五題內完成。 |
...
| `staff_review_summary.objective` | array | yes | iMVS 量測到的客觀資訊，例如 heart rate、SpO2、temperature 等 vital payload 摘要。 | 範例：`["Demo vital payload includes HR 150 bpm, SpO2 98%, BP 102/68 mmHg, and temperature 36.5 C."]`；整理 iMVS 量測資料。 |
