...
| CORS origins | 第一輪 rehearsal allow browser origins `http://localhost` 與 `http://localhost:5174`；這是 iMVS/frontend 測試機瀏覽器頁面的 Origin，不是 Render localhost。 |
...
| Pending UI state | iMVS 送出答案後進入 pending answer state：snapshot answer body + `idempotency_key`，disable / readonly 當題答題 controls，只用同一 body/key retry，等 NYCU 下一題或 summary 回來再開啟下一個畫面的答題 controls。 |
...
目前 NYCU rehearsal backend 的執行位置如下：

- iMVS / browser 連到 NYCU-hosted Render HTTPS API。
- Render 雲端 Web Service 從 GitHub `main` branch 拉程式碼並執行 Node API。
- GitHub 只保存程式碼與部署版本，不負責 API 運算。
- NYCU / Jason 的本機電腦只用於開發、測試與更新；demo 呼叫不會連到本機電腦。
- Render service live 時，即使本機電腦關機，慧誠智醫仍可呼叫 Render API。
...
1. `request_id` / `idempotency_key`
   ...
   - 六月 demo 的 `idempotency_conflict` recovery 定調為 restart demo session：
     ...
     到已標示的 fallback。
   - iMVS 前端送出答案後，建議進入 pending answer state：立即 snapshot 當次
     `question_id`、answer body 與 `idempotency_key`，並把當題所有答題相關
     controls 設為 disabled / readonly；收到 NYCU 下一題或 summary 後，再開啟
     下一個畫面的答題控制。
...
4. Demo environment
   ...
   - API 執行在 Render 雲端，不是 NYCU / Jason 本機；GitHub 只保存 code，
     Render 會從 GitHub `main` 拉 code 並執行 Node service。
   ...
   - NYCU 需開放 CORS origins：
     ...
     這兩個值是 locally served imedtac/iMVS frontend 的 browser
     `Origin`，不是 Render localhost；若實際測試環境使用
     `127.0.0.1`、內網 IP、其他 port、HTTPS domain 或 WebView custom
     origin，請提供實際 Origin header 後再加入 allowlist。
   - Demo bearer token 採 NYCU 環境變數控制做法：若 NYCU 在 Render 設定
     `DEMO_BEARER_TOKEN`，兩個 POST endpoints 會要求
     `Authorization: Bearer <demo token>`；`GET /healthz` 與 `OPTIONS`
     preflight 不要求 token。Jason 已在 Render Environment 準備
     `DEMO_BEARER_TOKEN`；實際 token 不寫入 repo 文件，另由私下通道提供。
     Token-required mode 需在 Render 儲存環境變數並重新 deploy 最新
     GitHub `main` commit 後才會生效。
...
-> disable / readonly all answer-related controls for the current question
-> retry only the same body with the same idempotency_key if needed
-> keep help / restart demo / operator fallback controls available if desired
-> unlock the next screen's answer controls only after NYCU returns the next
   question or summary
```

Professional implementation detail:

- `answer-related controls` 包含 option buttons、submit、back-to-edit、
  change-answer、next-answer，以及任何會改變當題 answer state 的控制。
- `non-answer controls` 可包含 help、restart demo、operator fallback、取消 demo
  等不會修改當題答案的控制。
- 若 timeout 後需要 retry，iMVS 只能 retry 同一份 answer body 與同一個
  `idempotency_key`；不可在同一 pending attempt 內修改 answer body。
- 若收到 `idempotency_conflict`，iMVS 不自動換新的 `idempotency_key` 重送不同
  答案；六月 demo recovery 是 restart demo session 或切換到已標示的 fallback。
- 這是前端 state-management / duplicate-submit prevention 建議，不新增 endpoint，
  也不改兩個 endpoint 的主 API 架構。
